...
(() => {
    ...
    async function handle(msg) {
        ...
        case "bots": {
            const answer = await ask("bots", "bots");
            if (answer && answer.type === "failed") {
                return { error: `Клиент игры не отдал отношения — возможно, его обновили. (${answer.error})` };
            }
            if (!answer || !answer.bots) {
                return { error: "Партия ещё загружается — попробуйте через несколько секунд." };
            }
            return { bots: answer.bots };
        }
        case "relate": {
            const answer = await ask("relate", "related", { relation: msg.relation });
            if (answer && answer.type === "failed") {
                return { error: `Игра не приняла смену отношений. (${answer.error})` };
            }
            if (!answer || !answer.ok) {
                return { error: "Партия ещё загружается — попробуйте через несколько секунд." };
            }
