...
// HeroMark — герой игрока: какого он уровня и сколько человек это отметили.
// Отметка личная, как у признаков: героя видят в партии разные люди
// и в разное время, а уровень со временем растёт.
type HeroMark struct {
	// UnitTypeID — номер типа юнита: им герой опознаётся и в партии,
	// и в справочнике.
	UnitTypeID int
	// Level — сводный уровень: тот, который назвали чаще других. При
	// ничьей берётся больший — герой качается, а не разучивается.
	// Ноль означает, что героя никто не отмечал.
	Level int
	// Count — сколько человек отметили этого героя у игрока.
	Count int
	// Mine — уровень, который поставил я. Ноль — не отмечал.
	Mine int
}
