...
var messages = map[string]entry{
	...
	"faq.plan.a4":   {ru: "Ультра открывает ещё одно: карты партий, объявленных анонимными. Это событийные партии, в которых игра подменяет имя каждого участника на «анонимный». Скрыт при этом только ник — номер аккаунта настоящий, и состав с сайта по нему называет живого человека, так что на карте мы видим всех поимённо. Кому такая карта не открыта, тот увидит вместо неё объяснение, а проверка карты за это не тратится.", en: "Ultra opens one more thing: the maps of games declared anonymous. These are event games where the game replaces every participant’s name with “anonymous”. Only the nickname is hidden — the account number is real, and the roster from the site names a living person by it, so on the map we see everyone by name. Anyone without access sees an explanation instead of the map, and no map check is spent for it."},
