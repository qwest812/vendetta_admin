...
func TestGameStateBuild(t *testing.T) {
	...
	// Море и ничейная земля не должны попадать в провинции с владельцем,
	// а служебный игрок -1 — в список игроков.
	if len(g.Provinces) != 3 {
		t.Errorf("провинций с владельцем = %d, ожидалось 3", len(g.Provinces))
	}
