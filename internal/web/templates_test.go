...
func TestPagesRender(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			// Список своих провинций с моралью со страницы убран: она про то,
			// кто в партии играет, а не про свои клетки. Сколько их —
			// осталось строкой.
			want: []string{"вы играете за Франция", "номер в партии 29", "Провинций под контролем 1 из 2"},
			deny: []string{"Призвать сейчас", "призыв пехоты", "аккаунт проекта",
				"Париж", "столица", "Мораль"},
		},
		{
		...
		},
		{
			...
			// Состояния партии нет — остаются сведения из лобби.
